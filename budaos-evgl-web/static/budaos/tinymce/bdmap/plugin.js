tinymce.PluginManager.add('bdmap', function(editor, url) {
	let pluginName='插入百度地图';
	let baseURL=tinymce.baseURL;
    baseURL = baseURL.replace('/static/js','');
	let iframe1 = baseURL+'/static/budaos/tinymce/bdmap/map.html';
	let bdmap_width = function (editor) {
		return editor.getParam('bdmap_width', 560);
    };
    let bdmap_height = function (editor) {
	...
	let openDialog = function() {
		return editor.windowManager.openUrl({
			...
			onAction: function (api, details) {
				switch (details.name) {
					case 'save':
						html='<iframe src="'+baseURL+'/static/budaos/tinymce/bdmap/bd.html?center='+tinymceLng+'%2C'+tinymceLat+'&zoom=19&width='+(bdmap_width(editor)-2)+'&height='+(bdmap_height(editor)-2)+'" frameborder="0" style="width:'+bdmap_width(editor)+'px;height:'+bdmap_height(editor)+'px;">';
	...
	return {
		getMetadata: function() {
			return  {
				name: pluginName,
				url: "//tinymce.ax-z.cn/more-plugins/bdmap.php",
			};
		}
	};
});
